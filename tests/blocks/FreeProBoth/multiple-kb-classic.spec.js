...
test.beforeEach(async ({ page }) => {
  await page.goto(`${process.env.BASE_URL_MSF}/multiple-kb-classic/`);
  await page.waitForLoadState("domcontentloaded");
});

test("Multiple KB Classic Heading", async ({ page }) => {
  const mainHeading = page.locator(".entry-title");
  await expect(mainHeading).toBeVisible();
  await expect(mainHeading).toHaveText("Multiple KB Classic");
});
